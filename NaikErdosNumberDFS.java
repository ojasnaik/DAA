...
public class NaikErdosNumberDFS {
...
//        Collections.sort(erdosNumber123.get(1));
...
//        Collections.sort(erdosNumber123.get(2));
...
//        Collections.sort(erdosNumber123.get(3));
...
//        erdosNumber123.put(1, new ArrayList<>());
//        erdosNumber123.put(2, new ArrayList<>());
//        erdosNumber123.put(3, new ArrayList<>());
        ...
        for (int key : graph.keySet()) {
            if (!erdosNumber.containsKey(key)) {
                visited[key] = true;
                int erdos = getErdosNumber(key);
                visited[key] = false;
//                if (!erdosNumber.containsKey(key) && erdos != -1){
//                    erdosNumber.put(key, erdos);
//                    erdosNumber123.get()
//                }
            }
        }
    ...
    private static int getErdosNumber(int key) {
        if (key == 1)
            return 0;
        if (erdosNumber.containsKey(key)) {
            return erdosNumber.get(key);
        }
        int minErdosNumberForKey = Integer.MAX_VALUE;
        for (int neighbor : graph.get(key)) {
            if (!visited[neighbor]) {
                visited[neighbor] = true;
                int erdosNumberNeighbor = getErdosNumber(neighbor);
                visited[neighbor] = false;
                if (erdosNumberNeighbor != -1) {
                    int erdosNumberKey = 1 + erdosNumberNeighbor;
                    minErdosNumberForKey = Math.min(erdosNumberKey, minErdosNumberForKey);
                }
            }
        }
        if (minErdosNumberForKey != Integer.MAX_VALUE) {
            erdosNumber.put(key, minErdosNumberForKey);
            if (minErdosNumberForKey >= 1 && minErdosNumberForKey <= 3)
                erdosNumber123.get(minErdosNumberForKey).add(key);
            return minErdosNumberForKey;
        }
        return -1;
    }
